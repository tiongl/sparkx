...
import com.sparkx.analysis.{Critical, Warning, Info}
...
class OverviewPage(parent: SparkXTab) extends WebUIPage("") {
  ...
  override def render(request: HttpServletRequest): Seq[Node] = {
    ...
    val suggestions = IssueDetector.suggestions(parent.sparkUI.store, parent.config)

    val content = if (issues.isEmpty && suggestions.isEmpty) {
      ...
      <div>
        ...
        {if (suggestions.nonEmpty)
          <div style="margin-top:24px">
            <h4>💡 Optimization Suggestions
              <span class="badge" style="margin-left:8px">{suggestions.size}</span>
            </h4>
            <p>
              Potential optimizations detected from SQL query plan analysis.
              <a href={SparkXPageUtils.sparkxSubUrl(request, "suggestions")}>View details →</a>
            </p>
            <table class="table table-bordered table-condensed table-striped sortable">
              <thead>
                <tr>
                  <th>Severity</th><th>SQL Exec</th><th>Suggestion</th>
                  <th>Description</th>
                </tr>
              </thead>
              <tbody>
                {suggestions.map { s =>
                  ...
                  <tr>
                    ...
                    <td>{s.executionId match {
                      case Some(id) => <a href={SparkXPageUtils.sqlExecUrl(request, id)}>#{id}</a>
                      case None     => scala.xml.Text("—")
                    }}</td>
                    <td><a href={SparkXPageUtils.sparkxSubUrl(request, "suggestions")}>
                      <strong>{s.title}</strong></a></td>
                    <td>{s.description}</td>
                  </tr>
                }}
              </tbody>
